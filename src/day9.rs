...
fn check_neighbor(point: (usize,usize), row_max: usize, col_max: usize,map: &Vec<Vec<u8>>) -> Option<Vec<(usize,usize)>>{
...
fn  map_basin(point: (usize,usize), map: &Vec<Vec<u8>>, neighbors: &mut HashSet<(usize,usize)>) -> usize {
    let row_max = map.len() - 1;
    let col_max = map[0].len() - 1;
    ...
    match n {
        Some(mut x) => {            
            x = x.iter().filter(|t| neighbors.insert(**t)).map(|t| *t).collect();
            for i in x {
