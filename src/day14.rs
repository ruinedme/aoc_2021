...
fn get_min_max_difference(
    ...
    let mut letter_count: HashMap<char, usize> =
        HashMap::with_capacity((rules.len() as f64).sqrt() as usize);
    pair_count.iter().for_each(|x| {
        let c = x.0.char_indices().next().unwrap().1;
        *letter_count.entry(c).or_insert(0) += x.1;
    });

    //take last letter of the template and add 1 to letter count
